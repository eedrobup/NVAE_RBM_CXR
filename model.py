...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance,cond_z_dim=0):
        ...
        # If cond_z_dim > 0, we create a small network to influence q-distributions:
        self.cond_z_dim = cond_z_dim
        if self.cond_z_dim > 0:
            # This linear layer maps cond_z to shifts in mu and log_sigma for each scale's first group.
            # Adjust this dimension as needed. For a large model, you may need multiple mappings.
            # Here we assume a single global shift just as an example.
            latent_per_group = self.num_latent_per_group
            self.cond_mapper = nn.Sequential(
                nn.Linear(self.cond_z_dim, 2 * latent_per_group),
                nn.Tanh()
            )
    ...
    def forward(self, x, cond_z=None):
        s = self.stem(2 * x - 1.0)
        ...
        # Integrate cond_z if provided
        if cond_z is not None and self.cond_z_dim > 0:
            # Assume global conditioning: broadcast cond_z across spatial dims if needed.
            # If mu_q/log_sig_q have shape B, C, H, W and cond_z is [B, cond_z_dim],
            # we can map cond_z -> shift and add to mu_q, log_sig_q.
            shift = self.cond_mapper(cond_z)  # [B, 2*latent_per_group]
            # Reshape shift to [B, 2*latent_per_group, 1, 1]
            shift = shift.unsqueeze(-1).unsqueeze(-1)
            # Add shifts only to the first group for simplicity. For more advanced integration,
            # you might design a more complex conditioning scheme.
            mu_shift, log_sig_shift = torch.chunk(shift, 2, dim=1)
            mu_q = mu_q + mu_shift
            log_sig_q = log_sig_q + log_sig_shift
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    if cond_z is not None and self.cond_z_dim > 0:
                        # Example: apply the same shift logic at each group (optional)
                        shift = self.cond_mapper(cond_z)  # same shift used above or separate mappers per group
                        shift = shift.unsqueeze(-1).unsqueeze(-1)
                        mu_shift, log_sig_shift = torch.chunk(shift, 2, dim=1)
                        mu_q = mu_q + mu_shift
                        log_sig_q = log_sig_q + log_sig_shift
    ...
    def sample(self, num_samples, t, cond_z=None):
        scale_ind = 0
        z0_size = [num_samples] + self.z0_size
        dist = Normal(mu=torch.zeros(z0_size).to(DEVICE), log_sigma=torch.zeros(z0_size).to(DEVICE), temp=t)
        z, _ = dist.sample()
        ...
        if cond_z is not None and self.cond_z_dim > 0:
            ...
            shift_all = self.cond_mapper(cond_z)  # [B, 2*latent_per_group]
